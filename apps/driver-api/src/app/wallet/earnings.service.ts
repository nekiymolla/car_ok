    ...
    async getStats(driverId: number, timeFrame: TimeQuery): Promise<StatisticsResult> {
        ...
        switch (timeFrame) {
            case TimeQuery.Daily:
                dataset = await this.requestRepository.query(`SELECT CONCAT(ANY_VALUE(MONTH(requestTimestamp)),'/',ANY_VALUE(DAY(requestTimestamp))) as name, CONCAT(ANY_VALUE(MONTH(CURRENT_TIMESTAMP)),'/',ANY_VALUE(DAY(CURRENT_TIMESTAMP))) AS current, ${fields} from request WHERE DATEDIFF(NOW(),requestTimestamp) < 7 AND driverId = ? AND currency = ? GROUP BY DATE(requestTimestamp)`, [driverId, mostUsedCurrency]);
                break;
